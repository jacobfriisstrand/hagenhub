...
export default function AddListingTypeRadioButton({ value, formLabel, icon, isChecked }: AddListingTypeRadioButtonProps) {
  ...
  return (
    <div className="group flex flex-col relative justify-between rounded-md w-full aspect-video lg:aspect-square border border-gray-200 hover:border-gray-300 [&:has([data-state=checked])]:border-blue-500 [&:has([data-state=checked])]:ring-2 [&:has([data-state=checked])]:ring-blue-500 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500 transition-[border,ring] duration-200 lg:w-42">
      <RadioGroupItem
        id={id}
        value={value}
